...
class OrderProvider extends ChangeNotifier {
  ...
  String get pagoMovilTelefonoForm => dotenv.env['PAGO_MOVIL_TELEFONO_FORM'] ?? '04148783374';
  ...
  final List<SauceOption> _sauces = [
    SauceOption(name: 'Todas las salsas (Ajo, Maíz, Ketchup, Mayonesa, Mostaza)'),
    SauceOption(name: 'Sin salsas'),
    SauceOption(name: 'Extra Ajo', extraCostUsd: 0.50),
    SauceOption(name: 'Extra Queso', extraCostUsd: 1.00),
  ];

  // --- Delivery Zones ---
  final List<DeliveryZone> _deliveryZones = const [
    DeliveryZone(name: 'Retiro en local', costUsd: 0),
    DeliveryZone(name: 'Tipuro', costUsd: 2),
    DeliveryZone(name: 'Viboral', costUsd: 4.5),
  ];
  ...
  double get saucesExtraUsd {
    ...
    for (final sauce in _sauces) {
      if (sauce.selected) total += sauce.extraCostUsd;
    }
    return total;
  }

  double get deliveryCostUsd => _selectedZone?.costUsd ?? 0;
  ...
  String buildWhatsAppMessage({bool isPaid = false, String? reference}) {
    ...
    for (final item in _menuItems) {
      if (item.quantity > 0) {
        final lineTotal = item.totalUsd.toStringAsFixed(2);
    ...
    if (selectedSauces.isNotEmpty) {
      ...
      for (final sauce in selectedSauces) {
        buffer.writeln('• ${sauce.name} (${sauce.displayPrice})');
      }
      buffer.writeln('');
    }
    ...
    if (_selectedZone != null) {
      buffer.writeln(
          '• Zona: ${_selectedZone!.name} (${_selectedZone!.displayCost})');
    } else {
    ...
    if (!isPaid) {
      buffer.writeln(
          '📌 _Por favor adjunta tu ubicación y capture de pago._');
    } else {
      buffer.writeln(
          '📌 _Por favor adjunta tu ubicación._');
    }
  ...
  String getWhatsappUrlPaid(String reference) {
    final message = Uri.encodeComponent(buildWhatsAppMessage(isPaid: true, reference: reference));
    return 'https://wa.me/$whatsappNumber?text=$message';
  }
  ...
  Future<String?> verifyPayment(Map<String, dynamic> requestData) async {
    try {
      ...
      requestData['telefonoDestino'] = dotenv.env['TELEFONO_DESTINO'] ?? '04128798008';
      requestData['importe'] = '1.00';
      // Adding these just in case the API needs them, as requested by user
      requestData['cedulaDestino'] = dotenv.env['CEDULA_DESTINO'] ?? 'J506736853';
      ...
      await dio.post(
        ...
        options: Options(
          headers: {
            "X-Api-Key": dotenv.env['BDV_API_KEY'] ?? "9576945A28CC7EA4F67FF8179CC77ED1",
      ...
      switch (err.type) {
        ...
        case DioExceptionType.badResponse:
          ...
          if (data is Map<String, dynamic>) {
            return data["message"]?.toString() ?? "Error de validacion en el banco";
          }
